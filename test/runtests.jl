...
@testset "kge Function Tests" begin
    ...
    @test kge(observed, simulated, false, n_min=2) ≈ 0.9905 atol = 0.001
    @test kge(observed, simulated, true, n_min=2) ≈ 0.99318275 atol = 0.001
    ...
    @test kge(observed_with_novalue, simulated_with_novalue, false, no_value=no_value, n_min=2) ≈ 0.97297257 atol = 0.001
    @test kge(observed_with_novalue, simulated_with_novalue, true, no_value=no_value, n_min=2) ≈ 0.96132002 atol = 0.001
...
@testset "kge_ts Function Tests" begin
    ...
    @test kge_ts(observed_ta, simulated_ta, false, n_min=2) ≈ 0.9905 atol = 0.001
    @test kge_ts(observed_ta, simulated_ta, true, n_min=2) ≈ 0.99318275 atol = 0.001
...
@testset "Nash-Sutcliffe Efficiency Tests" begin
    ...
    nse_value = nse_ts(observed_data, simulated_data, n_min=2)
...
@testset "Log-Transformed Nash-Sutcliffe Efficiency Tests" begin
    ...
    # Test for exact timestamp match and close values
    nse_log_value = nse_log_ts(observed, simulated, n_min=2)
...
@testset "Mean Squared Error Function Tests" begin
    ...
    @test mse(observed, predicted, n_min=3) ≈ 0.0074999 atol = 0.001

    # Test 2: Different lengths should raise an error
    observed_short = [2.0, 3.0]
    @test_throws AssertionError mse(observed_short, predicted)

    # Test 3: Empty arrays should return NaN or some error
    @test_throws AssertionError mse(Float64[], Float64[])
end
...
@testset "Mean Squared Error for Time Series Function Tests" begin
    ...
    @test mse_ts(observed_ta, simulated_ta, n_min=5) ≈ 100 atol = 10
    ...
    @test_throws AssertionError mse_ts(observed_ta, simulated_ta_shifted)
    ...
    @test mse_ts(observed_ta, simulated_ta_no_values; no_value=-9999.0, n_min=5) ≈ 100 atol = 10
end